...
function buildRequestOptions(action, index = 0) {
  ...
  const headers = (0, _assign2.default)({}, process.env.hasOwnProperty('HEADERS') && process.env.HEADERS.length ? process.env.HEADERS : {}, endPoint[index].hasOwnProperty('headers') ? endPoint[index].headers : {}, payload && action.payload.hasOwnProperty('JCAC_headers') ? action.payload.JCAC_headers : {});
  const query = payload && action.payload.hasOwnProperty('JCAC_query') ? _querystring2.default.stringify(action.payload.JCAC_query) : {};
  const URL = [process.env.hasOwnProperty('API_HOST') && process.env.API_HOST.length ? process.env.API_HOST : '', endPoint[index].url, payload && action.payload.hasOwnProperty('JCAC_url') ? action.payload.JCAC_url : '', payload && action.payload.hasOwnProperty('JCAC_query') ? _querystring2.default.stringify(action.payload.JCAC_query) : '', !checkDataTransformReq(method) ? buildQueryParams(action.payload, query) : ''].join('');
...
const singleRequestBuilder = (action, store, next) => {
  ...
  const reqOptions = (0, _assign2.default)(buildRequestOptions(action), checkDataTransformReq(endPoint[0].method) ? { data: (0, _stringify2.default)(reduceDataKeys(action.payload)) } : {});
...
const multipleRequestBuilder = (action, store, next) => {
  ...
  _promise2.default.all(endPoint.map((call, i) => {
    ...
    const reqOptions = (0, _assign2.default)(buildRequestOptions(action, i), checkDataTransformReq(i.method) ? { data: (0, _stringify2.default)(reduceDataKeys(action.payload)) } : {});
